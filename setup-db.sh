...
echo "Настройка базы данных для kursach_stas..."
echo "=========================================="
...
if ! psql -h $DB_HOST -p $DB_PORT -U $POSTGRES_USER -c '\q' 2>/dev/null; then
    ...
    if ! psql -h $DB_HOST -p $DB_PORT -U $POSTGRES_USER -c '\q' 2>/dev/null; then
        ...
        # Если подключение без пароля успешно, устанавливаем пароль для пользователя postgres
        echo "Установка пароля для пользователя $POSTGRES_USER..."
        psql -h $DB_HOST -p $DB_PORT -U $POSTGRES_USER -c "ALTER USER $POSTGRES_USER WITH PASSWORD '$POSTGRES_PASSWORD';" 2>/dev/null
        if [ $? -eq 0 ]; then
            echo "Пароль для $POSTGRES_USER установлен ✓"
        fi
        export PGPASSWORD="$POSTGRES_PASSWORD"
    fi
fi
